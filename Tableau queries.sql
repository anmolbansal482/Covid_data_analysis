...
-- 8 own queries for extra tableau visualisations

with cte
as
(
select location, date, total_cases, coalesce(lag(total_cases) OVER (ORDER by date)) as lagged_value,
total_cases - coalesce(lag(total_cases) OVER (ORDER BY date)) as difference,
(total_cases - coalesce(lag(total_cases) OVER (ORDER BY date))) / coalesce(lag(total_cases) 
	OVER (ORDER BY date)) * 100 as Percentage_Change
from coviddeaths
where location not in ('World', 'European Union', 'International', 'Asia', 'North America', 'South America', 'Africa') 
	AND location = 'Mexico'
order by 1, 2
)
select * 
